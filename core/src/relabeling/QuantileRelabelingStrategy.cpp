...
std::unordered_map<size_t, Eigen::VectorXd> QuantileRelabelingStrategy::relabel(
    const std::vector<size_t>& samples,
    const Observations& observations) {

  std::vector<double> sorted_outcomes;
  for (size_t sample : samples) {
    sorted_outcomes.push_back(observations.get(Observations::OUTCOME, sample)(0));
  }
  ...
  for (size_t sample : samples) {
    double outcome = observations.get(Observations::OUTCOME, sample)(0);
    auto quantile = std::lower_bound(quantile_cutoffs.begin(),
                                     quantile_cutoffs.end(),
                                     outcome);
    long quantile_index = quantile - quantile_cutoffs.begin();
    relabeled_observations[sample] = Eigen::VectorXd::Constant(1, (uint) quantile_index);
  }
