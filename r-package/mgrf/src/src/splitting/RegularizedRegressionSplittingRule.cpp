...
bool RegularizedRegressionSplittingRule::find_best_split(size_t node,
  ...
  Eigen::VectorXd sum_node = Eigen::VectorXd::Zero(labels_by_sample.begin()->second.size());
  for (auto& sample : samples[node]) {
    sum_node += labels_by_sample.at(sample);
  }

  // Compute the outcome variance within this node.
  Eigen::VectorXd node_mean = sum_node / num_samples_node;
  double node_impurity = 0.0;
  for (auto& sample : samples[node]) {
    Eigen::VectorXd difference = labels_by_sample.at(sample) - node_mean;
    node_impurity += difference.norm() * difference.norm();
  }
...
void RegularizedRegressionSplittingRule::find_best_split_value_small_q(size_t node, size_t var, Eigen::VectorXd sum_node,
  ...
  Eigen::VectorXd* sums_right;
  size_t* n_right;
  sums_right = sums;
  n_right = counter;
  std::fill(sums_right, sums_right + num_splits, Eigen::VectorXd::Zero(responses_by_sample.begin()->second.size()));
  std::fill(n_right, n_right + num_splits, 0);

  // Sum in right child and possible split
  for (auto& sample : samples[node]) {
    double value = data->get(sample, var);
    Eigen::VectorXd response = responses_by_sample.at(sample);
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    Eigen::VectorXd sum_right = sums_right[i];
    Eigen::VectorXd sum_left = sum_node - sum_right;
    double decrease = sum_left.squaredNorm() / (double) n_left + sum_right.squaredNorm() / (double) n_right[i];
...
void RegularizedRegressionSplittingRule::find_best_split_value_large_q(size_t node, size_t var, Eigen::VectorXd sum_node,
                                                                       size_t num_samples_node,
                                                                       double node_impurity,
                                                                       double& best_value,
                                                                       size_t& best_var,
  ...
  std::fill(sums, sums + num_unique, Eigen::VectorXd::Zero(responses_by_sample.begin()->second.size()));
  ...
  size_t n_left = 0;
  Eigen::VectorXd sum_left = Eigen::VectorXd::Zero(responses_by_sample.begin()->second.size());
  ...
  for (size_t i = 0; i < num_unique - 1; ++i) {
    ...
    double decrease = sum_left.squaredNorm() / (double) n_left + sum_right.squaredNorm() / (double) n_right;
