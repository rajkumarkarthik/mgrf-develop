...
#' We also allow for multiple treatment arms by letting W be fed in 
#' as a matrix of dimensions number of obs x number of treatment arms.
#' In this case, the average partial effects take the form
#' Var[W | X = x]^-1 * Cov[W, Y | X = x].
#'
#' @param X The covariates used in the causal regression.
#' @param Y The outcome.
#' @param W The treatment assignment (may be binary or real). For multiple treatment arms,
#'          feed as a matrix.
...
causal_forest <- function(X, Y, W, sample.fraction = 0.5, mtry = ceiling(2*ncol(X)/3), 
  ...
  variable.names <- c(colnames(X), "outcome", colnames(W))
