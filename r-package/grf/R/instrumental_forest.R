...
instrumental_forest <- function(X, Y, W, Z, sample.fraction = 0.5, mtry = ceiling(2*ncol(X)/3), 
    ...
    if(is.vector(W)) {
      W <- matrix(W, ncol=1)
      colnames(W) <- "treatment"
    }
    if(nrow(W) != nrow(X)) { stop("W has incorrect length.") }
    if(is.vector(Z)) {
      Z <- matrix(Z, ncol=1)
      colnames(Z) <- "treatment"
    }
    if(nrow(Z) != nrow(X)) { stop("Z has incorrect length.") }
    ...
    if (!precompute.nuisance) {
        ...
        forest.W.list <- lapply(seq_len(ncol(W)), function(i) {regression_forest(X, W[, i], sample.fraction = sample.fraction, mtry = mtry, num.trees = min(500, num.trees), num.threads = num.threads, min.node.size = NULL, honesty = TRUE, seed = seed, ci.group.size = 1, alpha = alpha, lambda = lambda, downweight.penalty = downweight.penalty)})
        W.hat <- sapply(forest.W.list, function(forest.W) {predict(forest.W)$predictions})
        
        forest.Z.list <- lapply(seq_len(ncol(Z)), function(i) {regression_forest(X, Z[, i], sample.fraction = sample.fraction, mtry = mtry, num.trees = min(500, num.trees), num.threads = num.threads, min.node.size = NULL, honesty = TRUE, seed = seed, ci.group.size = 1, alpha = alpha, lambda = lambda, downweight.penalty = downweight.penalty)})
        Z.hat <- sapply(forest.Z.list, function(forest.Z) {predict(forest.Z)$predictions})
    ...
    outcome.index <- ncol(X) # Indicies in C++ format, not R.
    treatment.index <- ncol(X) + seq_len(ncol(W))
    instrument.index <- ncol(X) + ncol(W) + seq_len(ncol(Z))
